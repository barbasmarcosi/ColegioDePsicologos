...
function NewBillForm() {
  ...
  const onSubmit = (event) => {
    ...
    console.log(!Number(newBillNumber),newBillNumber.length)
    if (
      !(
        newBillNumber &&
        newBillDescription &&
    ...
    } else if (!(Number(newBillNumber) && newBillNumber.length === 13)) {
      alert("El numero de factura debe ser numerico y de 13 digitos");
    } else if (!(Number(newBillAmount))) {
    alert("El monto debe ser numerico y separando los decimales con '.'");
    } else {
      addBill(
        newBillNumber,
        newBillDescription,
